<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-button v-if="specsGroupManager_btn_add" class="filter-item" style="margin-left: 10px;" type="primary" icon="edit" @click="handleCreate">添加</el-button>
    </div>
    <el-table v-loading.body="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%">
      <el-table-column v-if="show" align="center" label="pk_specs_group_id">
        <template scope="scope">
          <span>{{ scope.row.pkSpecsGroupId }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      getObj(row.pkSpecsGroupId).then(response => {
    ...
    update(formName) {
      ...
      set[formName].validate(valid => {
        if (valid) {
          ...
          putObj(this.form.pkSpecsGroupId, this.form).then(() => {
            this.dialogFormVisible = false
            this.getList()
            this.$notify({
              title: '成功',
